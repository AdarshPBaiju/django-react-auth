    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                # Check if the token is expired or already blacklisted
                if 'Token is blacklisted' in str(e) or 'Token is invalid or expired' in str(e):
                    return Response({"message": "Logout successful."}, status=status.HTTP_200_OK)
                return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
            except AttributeError:
                return Response({"error": "Token blacklisting not enabled."}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
...
class VideoFileListAPIView(generics.ListAPIView):
    permission_classes = [AllowAny]
